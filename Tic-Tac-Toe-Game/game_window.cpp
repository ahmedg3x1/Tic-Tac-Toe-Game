...
int game_window::minimax(bool isMaximizingPlayer = false, int alpha = -10, int beta = 10) {
  if (checkGameState() != Continue_State) {
    if (winner == PlayerTwoBoardCode)
      return winScore;
    else if (winner == PlayerOneBoardCode)
      return loseScore;
    else
      return tieScore;
  }
...
int game_window::checkGameState() {
  ...
  for (int i = 0; i < 3; i++) {
    for (int j = 0; j < 3; j++)
      if (board[i][j] == EMPTY) return Continue_State;  // game is not finished yet;
  }
  winner = 0;
  return Tie_State;
}
...
void game_window::EnableSlots(bool state) {
  for (int i = 0; i < 3; i++)
    for (int j = 0; j < 3; j++) Slot[i][j]->setEnabled(state);
}

void game_window::on_Next_clicked() {
  int i;
  for (i = 0; i < 9; ++i)
    if (game.moves[i] == log_counter) break;
